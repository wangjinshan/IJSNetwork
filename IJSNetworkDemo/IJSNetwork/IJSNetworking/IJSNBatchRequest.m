...
- (instancetype)initWithRequestArray:(NSArray<IJSNCacheRequest *> *)requestArray {
    ...
    if (self) {
        ...
        for (IJSNCacheRequest *req in _requestArray) { //类型检查，所有元素都必须为IJSCacheRequest或的它的子类，否则强制初始化失败
            if (![req isKindOfClass:[IJSNCacheRequest class]]) {
                IJSNLog(@"请求的类必须是 IJSCacheRequest 对象");
                return nil;
            }
        }
    }
...
- (void)start {
    ...
    [[IJSNBatchRequestAgent sharedAgent] addBatchRequest:self]; //使用IJSBatchRequestAgent来管理当前的批量请求
    [self toggleAccessoriesWillStartCallBack];                  // 通知代理请求将要开始
    for (IJSNCacheRequest *req in _requestArray) {              //遍历所有request，并开始请求
        req.delegate = self;
...
- (void)requestFinished:(IJSNCacheRequest *)request {
    ...
    if (_finishedCount == _requestArray.count) //如果_finishedCount等于_requestArray的个数，则判定当前batch请求成功
    {
        ...
        [self toggleAccessoriesDidStopCallBack];
        [[IJSNBatchRequestAgent sharedAgent] removeBatchRequest:self]; //从IJSBatchRequestAgent里移除当前的batch
    }
}
