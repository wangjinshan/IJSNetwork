...
- (NSURLSessionTask *)sessionTaskForRequest:(IJSNBaseRequest *)request error:(NSError *_Nullable __autoreleasing *)error {
    ...
    switch (method) {
        ...
        case IJSRequestMethodPOST: { // post --- 包括post 上传任务
        ...
        case IJSRequestMethodHEAD: { // hrad
            return [self dataTaskWithHTTPMethod:@"HEAD" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        }
        case IJSRequestMethodPUT: { // put
            return [self dataTaskWithHTTPMethod:@"PUT" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        }
        case IJSRequestMethodDELETE: { //delte
            return [self dataTaskWithHTTPMethod:@"DELETE" requestSerializer:requestSerializer URLString:url parameters:param error:error];
        }
        case IJSRequestMethodPATCH: { //patch
...
- (BOOL)validateResult:(IJSNBaseRequest *)request error:(NSError *_Nullable __autoreleasing *)error {
    ...
    if (json && validator) {              //通过json和validator来判断json是否有效
...
- (void)requestDidSucceedWithRequest:(IJSNBaseRequest *)request {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (request.delegate != nil) {                  //如果有代理，则调用成功的代理
...
- (void)requestDidFailWithRequest:(IJSNBaseRequest *)request error:(NSError *)error {
    ...
    @autoreleasepool {
        [request requestFailedPreprocessor]; //请求失败的预处理，IJS没有定义，需要用户定义
    }
    dispatch_async(dispatch_get_main_queue(), ^{
        [request toggleAccessoriesWillStopCallBack]; //告诉Accessories请求就要停止了
...
- (NSString *)buildRequestUrl:(IJSNBaseRequest *)request {
    ...
    NSString *detailUrl = request.requestUrl; //用户自定义的url（不包括在IJSConfig里面设置的base_url）
    ...
    if (request.useCDN) {
        ...
        if ([request baseUrl].length > 0) { //如果使用baseUrl，在当前请求没有配置baseUrl，返回全局配置的baseUrl
